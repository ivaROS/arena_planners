"""State dataclasses for CrowdNav (SARL) inference."""
...
class FullState:
    """Full robot state including goal and preferred velocity."""
    ...
    def __str__(self):
        fields = [self.px, self.py, self.vx, self.vy, self.radius, self.gx, self.gy, self.v_pref, self.theta]
        return " ".join(str(x) for x in fields)


class ObservableState:
    """Observable pedestrian state."""
...
class JointState:
    """Robot state paired with all observable human states."""

    def __init__(self, self_state: FullState, human_states: list[ObservableState]):
